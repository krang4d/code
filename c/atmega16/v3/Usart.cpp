...
static int F_CPU = 8000000;
static int BAUD = 9600;
static int MYUBRR = 51;
...
static void USART_Transmit(byte data)
{
...
{
    byte *s = get_str();
    int n = sizeof cmd_list /sizeof cmd_list[0];

    for(i = 0; i <= n; i++)
	{
        if (strcmp(s, cmd_list[i]) == 0)
	    {
        ...
        else USART_Puts("error: bad command!");
	}
    free(s);
}
