...
commands::commands() {
    cmd[] = {
        ...
        "K15", "K16", "K17", "K18", "K19", "K20", "K21", "STAT"
        };          // возможно это надо заменить на перечисление enum и сделать, как отдельный тип или клас с методами set(), get()
    c_size = sizeof cmd /sizeof cmd[0];
}
...
size_t commands::get_size() {
    return c_size;
}

char* commands::get_name(int i) {
    if (i>0 && i <= c_size) {
        return cmd[i];
    }
    else return NULL;
